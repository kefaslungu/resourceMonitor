...
* Требуется NVDA версии 2025.3.2 или более поздней версии.
* Для объявления выпусков Windows Server NVDA будет использовать унифицированное название выпуска вместо специального серверного наименования (например, «Windows Server 24H2» вместо «Windows Server 2025»).
* NVDA больше не будет добавлять «0 минут» при озвучивании состояния заряда батареи (например: «4 часа» вместо «4 часа, 0 минут»).
* NVDA теперь распознаёт дополнительные приставки для обозначения больших объёмов памяти и данных, включая роннабайт (1000 в девятой степени) и кветтабайт (1000 в десятой степени).
