...
def getWinVer():
	...
	if currentWinVer >= winVersion.WIN10:
		...
		# These two conditions were formerly part of a separate function,
		# combined into this function in add-on version 22.01 so release name attribute can be checked.
		# Special cases: Windows 10 Version 1507, Windows Server long-term servicing channel (LTSC) releases.
		if isClient and currentWinVer.build == 10240:
			winverName = "Windows 10 1507"
		elif not isClient and currentWinVer.build in serverReleaseNames:
			winverName = serverReleaseNames[buildNum]
		...
		if isClient:
			if currentWinVer.build >= 22000:
				winverName = "Windows 11 {0}".format(releaseID)
			else:
				winverName = "Windows 10 {0}".format(releaseID)
		else:
			winverName = "Windows Server {0}".format(releaseID)
